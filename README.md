...
- fixed-steady-state JAX first-order structural likelihood and NumPyro wrappers that can run compiled kernels like `NUTS` on the parsed-model first-order path when the steady state is supplied explicitly
...
- fully JAX-traceable parsed-model structural likelihoods with automatic steady-state and calibration solves for compiled NumPyro kernels like `NUTS` and `HMC`
- regime switching
