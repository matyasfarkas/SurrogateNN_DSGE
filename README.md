...
- parsed-model `get_irf` and `simulate_model` runtime helpers for first-order and deterministic SEP paths, including named shock selection, explicit shock histories, Julia-style `shocks="simulate"` random simulations with deterministic seeding, variable selection, and OBC-aware `ignore_obc` routing
