...
- tests verify a closed-form linear AR(1) IRF, simulation-vs-rollout parity on the same linear model, first-order OBC enforcement parity against deterministic SEP on a simple bound model, SEP fallback for terminal-state OBC requests, and runtime smoke across upstream `RBC_Dynare`, `FS2000`, and `RBC_baseline`
