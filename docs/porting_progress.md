...
- first-order runtime requests on OBC models now honor `ignore_obc`; when `ignore_obc = false`, the helper uses a dedicated first-order enforcement path for direct current-variable `max` / `min` equations and otherwise falls back to a deterministic `SEPConfig(branching_order = 0)` solve instead of silently returning the unconstrained linear path
