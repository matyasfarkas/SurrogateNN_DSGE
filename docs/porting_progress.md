...
- first-order runtime requests on OBC models now honor Julia-style `ignore_obc` semantics: when OBC-tagged shocks are present the flag is overridden and constraints are still enforced, while `ignore_obc = false` uses a dedicated first-order enforcement path for direct current-variable `max` / `min` equations and otherwise falls back to a deterministic `SEPConfig(branching_order = 0)` solve instead of silently returning the unconstrained linear path
- the dedicated first-order OBC path now recovers and returns implied OBC shock sequences when explicit OBC shock channels are present in the parsed model, which moves the runtime behavior closer to the Julia “constraints are enforced using shocks” surface without yet claiming the full horizon optimizer
- the dedicated first-order OBC path projects supported constrained variables period by period and matches deterministic SEP on the simple parsed bound models already used in the regression suite, while still preserving the existing SEP fallback for terminal-state requests and more general OBC equation shapes
- tests verify a closed-form linear AR(1) IRF, simulation-vs-rollout parity on the same linear model, first-order OBC enforcement parity against deterministic SEP on a simple bound model, implied OBC shock recovery on the dedicated first-order path, SEP fallback for terminal-state OBC requests, and runtime smoke across upstream `RBC_Dynare`, `FS2000`, and `RBC_baseline`
...
### 43. Automatic filter-gated switching likelihoods and NumPyro wrappers
...
- `switching_loglikelihood_from_model_filter_gates_jax` is implemented, composing the compiled first-order filter-derived gate statistics, JAX gate-probability construction, and the compiled first-order switching likelihood in one wrapper
- `build_numpyro_switching_filter_model_jax` and `evaluate_numpyro_switching_filter_log_density_jax` are implemented, so the first-order switching-order estimation path can now run in NumPyro without precomputing `gate_probs` outside the trace
- the new automatic-gating wrapper supports both inversion and Kalman gate filters, the same Schur/doubling first-order solution choice, automatic JAX steady-state/calibration solves, and the existing switching mixture controls
...
- `homotopy_chained_trajectory(...)` is now implemented on top of that path, reusing `homotopy_sep(...)` period by period to build nonlinear trajectories from explicit or simulated shock sequences
...
- parsed `precompile = true` directives on `@model` and `@parameters` now trigger eager symbolic/JAX cache construction at parse time, with derivative precompilation respecting `perturbation_order`
...
- the steady-state and calibrated-parameter Newton path now has symbolic-Jacobian safety fallbacks plus restart heuristics on both the NumPy and JAX implementations, including sign-preserving unit-scale feasibility seeds when the model-specific default guess family stays outside the residual domain, so non-finite default guesses and singular autodiff Jacobians no longer fail immediately on the compiled first-order path
...
- the steady-state Newton stack now also has a final square-system residual homotopy continuation fallback on both the NumPy and JAX paths, used only after the restart and least-squares layers fail; it walks from a trivial anchor problem to the true target residual system and only replaces the incumbent when the final target stage actually converges
...
- The current parsed front end does not yet port the remaining non-equation `@parameters` directives from the Julia macro layer beyond `guess`, bounds, `silent`, `symbolic`, `perturbation_order`, `precompile`, and the common `simplify` / `verbose` options.
- The current parsed front end now supports basic `max` / `min` OBC syntax, branch-frozen linearization around the active steady-state branch, parsed-model OBC violation diagnostics, and a dedicated first-order enforcement path for direct, simple monotone-transformed, and simple complementarity-style current-variable OBC equations. That first-order path now includes receding-horizon OBC shock-sequence optimization across `max_obc_horizon + 1` periods when explicit OBC shock channels are present, but the broader Julia-specific enforcement layer around kinks, including the remaining runtime surface and subdifferential Newton variants beyond that subset, is still unported.
- Parsed-model `get_irf` and `simulate_model` are now available, including Julia-style `shocks = :simulate` random-shock semantics, grouped nested runtime name selections, and the main selector tokens `:all_excluding_obc` / `:all_excluding_auxiliary_and_obc`, but the full Julia runtime surface is still broader because general first-order OBC enforcement still falls back to deterministic SEP outside the newly supported direct / monotone-transformed / simple complementarity subset.
